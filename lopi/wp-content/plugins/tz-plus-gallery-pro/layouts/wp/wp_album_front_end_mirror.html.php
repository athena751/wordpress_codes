...
function wp_front_end_base_album_mirror($album_ids, $options_color, $options_columns, $options_padding,
                                        $options_height,  $options_album1, $desk, $tablet, $mobile)
{
    ...
    wp_enqueue_style("plusgallery_aragon", plugins_url("tz-plus-gallery-pro/css/pro/mirror.css"), FALSE);
            ...
            <?php foreach($wpimages as $wpimage): ?>
                ...
                <li class="pgthumb mirror">
                    ...
                    <div class="view second-effect">
                        ...
                        <div class="back">
                            <a title="<?php echo esc_attr($wpimage->image_title);?>" class="info pgthumbimg <?php if(empty($wpimage->image_link)){ echo "pgthumbimg_one"; }?> "
                                             href="<?php echo esc_url($wpimage->image_url);?>" rel="prettyPhoto[<?php echo esc_attr($album_ids);?>]">
                                <i class="fa fa-search"></i>
                            </a>
                            <?php if($wpimage->image_link){ ?>
                            <a title="<?php echo esc_attr($wpimage->image_title);?>" class="image_url"
                               href="<?php echo esc_url($wpimage->image_link);?>"
                                <?php if($wpimage->link_target){ ?> target="_blank" <?php } ?>><i class="fa fa-link"></i>
                            </a>
                            <?php } ?>
                            <?php if($wpimage->image_title){ ?>
                            <span class="title"><?php echo esc_attr($wpimage->image_title);?></span>
                            <?php } ?>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        function tz_init(){
            ...
            TzPlusTemplateResizeImage(jQuery('.mirror'));
