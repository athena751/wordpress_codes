...
function wp_front_end_base_album_black($album_ids, $options_color, $options_columns, $options_padding,
                                       $options_height,  $options_album1, $desk, $tablet, $mobile)
{
    ...
    wp_enqueue_style("plusgallery_aragon", plugins_url("tz-plus-gallery-pro/css/pro/black_white.css"), FALSE);
    ...
    <div class="tzwpgallery plusgallery no-touch pg loaded" id="plusgallery<?php echo esc_attr($album_ids);?>">
        <ul class="clearfix tzmasonry" id="pgthumbs">
            <?php foreach($wpimages as $wpimage): ?>
                ...
                <li class="pgthumb black_white">
                    <div class="effect3">
                        <div class="black_white-overlay"></div>
                        <img title="<?php echo esc_attr($wpimage->image_title);?>" alt="<?php echo esc_attr($wpimage->image_title);?>" src="<?php echo esc_url($wpimage->image_url);?>">
                        <div class="black_white-info">
                            <?php if($wpimage->image_link){ ?>
                            <a href="<?php echo esc_url($wpimage->image_link);?>" <?php if($wpimage->link_target){ ?> target="_blank" <?php } ?>><?php echo esc_attr($wpimage->image_title);?></a>
                                <?php } else{ ?>
                                <a href="<?php echo esc_url($wpimage->image_url);?>" rel="prettyPhoto[<?php echo esc_attr($album_ids);?>]"><?php echo esc_attr($wpimage->image_title);?></a>
                                    <?php } ?>
    ...
    <script type="text/javascript">
        function tz_init(){
            ...
            TzPlusTemplateResizeImage(jQuery('.black_white'));
