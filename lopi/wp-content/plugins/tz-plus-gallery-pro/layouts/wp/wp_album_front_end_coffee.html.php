...
function wp_front_end_base_album_coffee($album_ids, $options_color, $options_columns, $options_padding,
                                        $options_height,  $options_album1, $desk, $tablet, $mobile)
{
    ...
    wp_enqueue_style("plusgallery_aragon", plugins_url("tz-plus-gallery-pro/css/pro/coffee.css"), FALSE);
    ...
    <div class="tzwpgallery plusgallery no-touch pg loaded" id="plusgallery<?php echo esc_attr($album_ids);?>">
        <ul class="clearfix tzmasonry" id="pgthumbs">
            <?php foreach($wpimages as $wpimage): ?>
                ...
                <li class="pgthumb coffee">
                    <div class="effect3">
                        <div class="coffee-overlay"></div>
                        <img alt="<?php echo esc_attr($wpimage->image_title);?>" src="<?php echo esc_url($wpimage->image_url);?>">
                        <div class="corner-left"></div>
                        <div class="corner-right"></div>
                        <div class="corner-bottom"></div>
                        <div class="coffee-info">
                            <?php if($wpimage->image_link){ ?>
                                <a class="info pgthumbimg" href="<?php echo esc_url($wpimage->image_link);?>" <?php if($wpimage->link_target){ ?> target="_blank" <?php } ?>>
                                    <span><?php echo esc_attr($wpimage->image_title);?></span>
                                </a>
                            <?php } else{ ?>
                                <a class="info pgthumbimg" href="<?php echo esc_url($wpimage->image_url);?>" rel="prettyPhoto[<?php echo esc_attr($album_ids);?>]">
                                    <span><?php echo esc_attr($wpimage->image_title);?></span>
                                </a>
                            <?php } ?>
                        </div>
    ...
    <script type="text/javascript">
        function tz_init(){
            ...
            TzPlusTemplateResizeImage(jQuery('.coffee'));
