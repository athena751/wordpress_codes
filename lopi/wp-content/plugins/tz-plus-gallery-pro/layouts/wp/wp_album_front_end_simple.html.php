...
function wp_front_end_base_album_simple($album_ids, $options_color, $options_columns, $options_padding,
                                        $options_height,  $options_album1, $desk, $tablet, $mobile)
{
    ...
    wp_enqueue_style("plusgallery_aragon", plugins_url("tz-plus-gallery-pro/css/pro/simple.css"), FALSE);
                            ...
                            <div class="simple-info">
                                ...
                                <?php if($wpimage->image_link){ ?>
                                    <a title="<?php echo esc_attr($wpimage->image_title);?>" class="info pgthumbimg"
                                       href="<?php echo esc_url($wpimage->image_link);?>" <?php if($wpimage->link_target){ ?> target="_blank" <?php } ?>><?php echo esc_attr($wpimage->image_title);?>
                                    </a>
                                <?php } else{?>
                                    <a title="<?php echo esc_attr($wpimage->image_title);?>" class="info pgthumbimg"
                                       href="<?php echo esc_url($wpimage->image_url);?>" rel="prettyPhoto[<?php echo esc_attr($album_ids);?>]"><?php echo esc_attr($wpimage->image_title);?>
                                    </a>
                                <?php } ?>
