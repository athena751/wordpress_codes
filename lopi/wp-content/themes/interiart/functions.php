...
function interiart_register_front_end_styles()
{
    ...
    if ( is_page_template('template-under-construction.php') ):
        wp_enqueue_style( 'jquery.mCustomScrollbar', get_template_directory_uri().'/css/jquery.mCustomScrollbar.css', false );
    endif;
    ...
    $tzoptionpage = '';
    if ( isset($_GET["page_option"]) && $_GET["page_option"] == 'rtl' ){
        $tzoptionpage = 1;
    }

    $typepage = ot_get_option('interiart_rtl');

    if($tzoptionpage == '1' || $typepage == '1'){
        wp_enqueue_style('interiart-rtl', get_template_directory_uri() . '/css/rtl.css', false );
    }
}
...
function interiart_register_front_end_scripts()
{
    ...
    if ( is_page_template('template-homepage.php') || is_tax( 'portfolio-category') || is_tax( 'portfolio-tags') ):
        wp_enqueue_script( 'jquery.magnific-popup.min', get_template_directory_uri().'/js/jquery.magnific-popup.min.js', array(), false, true );
    endif;

    if ( is_page_template('template-under-construction.php') ):
        wp_enqueue_script( 'tz-countdown', get_template_directory_uri().'/js/countdown.js', array(), false, true );
        wp_enqueue_script( 'tz-button-nav', get_template_directory_uri().'/js/button_nav.js', array(), false, true );
        wp_enqueue_script( 'tz-customscrollbar', get_template_directory_uri().'/js/jquery.mCustomScrollbar.js', array(), false, true );
        wp_enqueue_script( 'tz-under-construction', get_template_directory_uri().'/js/tz_under_construction.js', array(), false, true );
    endif;
    ...
    if ( is_page_template('template-portfolio.php') ):
        ...
        $interiart_Width_option_val = '';
        if($interiart_Width_option != ''){
            $interiart_Width_option_val = $interiart_Width_option[0];
        }
        ...
        wp_enqueue_script('video', get_template_directory_uri().'/js/video.js',array(),false,true);
        ...
        $interiart_variables_portfolio = array(
            ...
            'width_option'    =>    $interiart_Width_option_val,
        );
    ...
    if ( is_tax( 'portfolio-category') || is_tax( 'portfolio-tags') ):
        wp_enqueue_script( 'tzportfolio-cat', get_template_directory_uri().'/js/tzportfolio-cat.js', array(), false, true );
    endif;
...
function interiart_register_required_plugins() {
    ...
    $interiart_plugins = array(
        ...
        array(
            'name'               => 'TZ PlusGallery Pro', // The plugin name.
            'slug'               => 'tz-plus-gallery-pro', // The plugin slug (typically the folder name).
            'source'             => get_stylesheet_directory() . '/plugins/tz-plus-gallery-pro.zip', // The plugin source.
            'required'           => true, // If false, the plugin is only 'recommended' instead of required.
